...
class ScheduleHistoryMenu(CallbackData, prefix="admin_schedule_history"):
    menu: str = "history"
    page: int = 1


class ScheduleFileDetailMenu(CallbackData, prefix="admin_schedule_file_detail"):
    file_id: int
    page: int = 1


class ScheduleFileActionMenu(CallbackData, prefix="admin_schedule_file_action"):
    file_id: int
    action: str  # "restore" or "back"
    page: int = 1


class LocalFilesMenu(CallbackData, prefix="admin_local_files"):
    menu: str = "local"
    page: int = 1


class LocalFileDetailMenu(CallbackData, prefix="admin_local_file_detail"):
    file_index: int
    page: int = 1


class LocalFileActionMenu(CallbackData, prefix="admin_local_file_action"):
    file_index: int
    action: str  # "delete", "rename", "recover", or "back"
    page: int = 1


class FileVersionsMenu(CallbackData, prefix="admin_file_versions"):
    filename: str
    page: int = 1


class FileVersionSelectMenu(CallbackData, prefix="admin_version_select"):
    file_id: int
    filename: str
    page: int = 1


class RestoreConfirmMenu(CallbackData, prefix="admin_restore_confirm"):
    file_id: int
    filename: str
        ...
        buttons.append(
            [
                InlineKeyboardButton(
                    ...
                    callback_data=f"admin_download_db:{file.id}",
                )
            ]
        )
    buttons.append(
    ...
    for file in schedule_files:
        buttons.append(
            [
                InlineKeyboardButton(
                    text=f"📥 {file}",
                    callback_data=f"admin_send_local:{file}",
                )
            ]
        )
    buttons.append(
...
def schedule_file_detail_kb(file_id: int, page: int) -> InlineKeyboardMarkup:
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton(
                text="📥 Скачать файл",
                callback_data=f"admin_download_db:{file_id}",
            )
        ],
        [
            InlineKeyboardButton(
...
def local_file_detail_kb(
    ...
    buttons = [
        ...
        [
            ...
            InlineKeyboardButton(
                text="📥 Скачать",
                callback_data=f"admin_send_local:{filename}",
            ),
        ],
        [
            InlineKeyboardButton(
