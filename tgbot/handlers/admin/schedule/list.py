...
from tgbot.misc.states.admin.schedule import RenameLocalFile

admin_list_router = Router()
admin_list_router.message.filter(F.chat.type == "private", AdministratorFilter())
admin_list_router.callback_query.filter(
    F.message.chat.type == "private", AdministratorFilter()
)


@admin_list_router.callback_query(ScheduleMenu.filter(F.menu == "local"))
...
@admin_list_router.callback_query(LocalFilesMenu.filter())
...
@admin_list_router.callback_query(ScheduleMenu.filter(F.menu == "history"))
...
@admin_list_router.callback_query(ScheduleHistoryMenu.filter())
...
@admin_list_router.callback_query(F.data.startswith("admin_send_local:"))
...
@admin_list_router.callback_query(ScheduleFileDetailMenu.filter())
...
@admin_list_router.callback_query(ScheduleFileActionMenu.filter())
...
@admin_list_router.callback_query(F.data.startswith("admin_download_db:"))
...
@admin_list_router.callback_query(LocalFileDetailMenu.filter())
...
@admin_list_router.callback_query(LocalFileActionMenu.filter())
...
@admin_list_router.message(RenameLocalFile.waiting_new_filename)
...
@admin_list_router.callback_query(FileVersionsMenu.filter())
...
@admin_list_router.callback_query(RestoreConfirmMenu.filter())
