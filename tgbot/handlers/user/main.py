...
from tgbot.keyboards.user.main import auth_kb, main_kb
...
@user_router.message(Authorization.email)
...
@user_router.message(Authorization.auth_code)
...
@user_router.message(Authorization.fullname)
...
Супер, авторизация пройдена. Теперь у тебя есть доступ ко всем ботам СТП 🥳""",
            ...
            await user_start(message=message, user=db_user)
            return
        else:
            await message.bot.edit_message_text(
                chat_id=message.chat.id,
...
Пользователем с такими ФИО уже зарегистрирован
Если ты считаешь, что произошла ошибка - напиши письмо на рассылку <code>СТП_РАССЫЛКА_МИП</code> с описанием ситуации

Либо можешь попробовать еще раз ввести ФИО
...
Пользователем с такими ФИО не найден в базе
Если ты считаешь, что произошла ошибка - напиши письмо на рассылку <code>СТП_РАССЫЛКА_МИП</code> с описанием ситуации
