...
class AdminFilter(BaseFilter):
    ...
    async def __call__(self, obj: Message, user: Employee, **kwargs) -> bool:
        """Проверка имеет ли пользователь роль Администратор.
...
class GokFilter(BaseFilter):
    ...
    async def __call__(self, obj: Message, user: Employee, **kwargs) -> bool:
        """Проверка имеет ли пользователь роль ГОК.
...
class MipFilter(BaseFilter):
    ...
    async def __call__(self, obj: Message, user: Employee, **kwargs) -> bool:
        """Проверка имеет ли пользователь роль МИП.
...
class HeadFilter(BaseFilter):
    ...
    async def __call__(self, obj: Message, user: Employee, **kwargs) -> bool:
        """Проверка имеет ли пользователь роль Руководитель.
...
class DutyFilter(BaseFilter):
    ...
    async def __call__(self, obj: Message, user: Employee, **kwargs) -> bool:
        """Проверка имеет ли пользователь роль Дежурный.
...
class SpecialistFilter(BaseFilter):
    ...
    async def __call__(self, obj: Message, user: Employee, **kwargs) -> bool:
        """Проверка имеет ли пользователь роль Специалист.
...
class MultiRoleFilter(BaseFilter):
    ...
    async def __call__(self, obj, **kwargs) -> bool:
        """Проверка соответствует ли пользователь хотя бы одной из переданных ролей.
